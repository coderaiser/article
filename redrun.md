...
`npm-run-all` очень не плохо себя показывает в плане удобства, но по принципу действия он не сильно уходить от `npm run`, каждая команда вызывается отдельно, что значительно его замедляет.
