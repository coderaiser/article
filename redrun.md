...
Я долгое время использовал `npm-run-all`, но в какой-то момент осознал, что запуск скриптов может быть более оптимальным, быстрым и обладающим простой реализацией. Первые результаты показали себя очень хорошо в плане скорости и эфективности, при этом не пренебрегая удобством.
