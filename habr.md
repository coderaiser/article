...
Cloud Commander разрабатывается в самом себе, как уже говорилось.
Но всё же, есть несколько вещей которые можно добавить. Проект хостится на [гитхабе](http://github.com/coderaiser/cloudcmd "Репозиторий").
В нём есть две ветки: **dev** и **master**. В первой находится вариант, который разрабатывается, туда вносятся все правки,
и хотя в идеале всё должно всегда работать, иногда случаются разные казусы. Во-второй же ветке находится стабильная версия.
Та, которая лежит в [npm](https://www.npmjs.org/package/cloudcmd "Node Package Manager").
...
тесты, а так же код выкладывается на [nodejitsu](https://www.nodejitsu.com/) и [Heroku](http://heroku.com).
И если что-то работает не так как нужно, сразу приходит письмо, а так же значёк в репозитории меняет свой цвет на красный.
...
Однажды мне попалась статься, в которой говорилось о
[стиле именования коммитов](http://habrahabr.ru/post/183646/ "Стиль именования коммитов").
На самом деле это очень важный процес, и сейчас, перед каждым релизом
ChangeLog генерируется одной командой: `gulp changelog`.
