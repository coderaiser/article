...
указателя текущего файла во время ввода имени (когда папок очень много, а музыки уменя много - листать список, не самое приятное из занятий).
...
- При отстутствии браузера с сервером можно будет общатся только по средством HTTP-запросов.
...
В таком большом приложении с огромным количеством модулей, было бы очень сложно разобратся,
если бы все файлы загружались сразу, при загрузке страницы. Такое приложение было бы
сложно поддерживать, поскольку время-от-времени одни программы обновлюятся, а другие устаревают
...
исполнятся, что значительно экономит увеличивает скорость работы, и экономит ресурсы как на клиенте,
так и на сервере.


### Join

Потоки `node.js` очень мощный инструмент, он кардинально отличается от того, что есть в других
скриптовых языках. В процессе изучения этого инструмента меня не покидала мысль о том, что
можно файлы обьединять в поток, и отдавать так, как буд-то файл один. Я думал, что будут задержки
в скорости, но нет. Всё работает как часы, и особых замедлений не ощущается, а в местое этого, появляется
...
Вкратце: если нужно загрузить файл `jquery.js` а и `jquery.fancybox.js`, это можно сделать таким образом:
```js
    http://cloudcmd.jit.su/join/lib/jquery.js:lib/fancybox.js
```
С помощью символа ":" имена файлов отделяются друг-от-друга, таким образом, обьединять можно абсолютно что угодно,
и на быстродействие сервера это не должно особо влияеть, поскольку файлы читаются последовательно,
