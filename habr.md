...
- [ElFinder](http://elfinder.org/) (содержит проблему 1 и 3 (а также 2, попробуйте скачать файл, и вы увидите попап, который нужно 
...
Важным отличием также является Node.js как платформа, на которой работает Файловый Менеджер.
...
- веб-сервер [express](http://expressjs.com/) который работает на порядок быстрее, чем встроенный;
- модуль [minify](https://github.com/coderaiser/minify), который минифицирует `js/css/html`;
...
Также обстоят дела и с директориями. Если опция включена, содержимое директории загружается единожды,
...
можно файлы объединять в поток, и отдавать так, как будто файл один. Я думал, что будут задержки
...
и на быстродействие сервера это не должно особо влиять, поскольку файлы читаются последовательно,
...
тесты, а также код разворачивается на NodeJitsu и Heroku.
...
Однажды мне попалась статья, в которой говорилось о
