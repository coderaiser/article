...
Некоторых из них уже не существует (Zip Magic 2000, например), другие же активно используються, и главное, разрабатываються по сей день (Total Commander).
...
(хотя объяснимо тем, что пишуться менеджеры в основном на серверных языках).

3. работают крайне медленно (не используют локальное хранилище и прочий HTML5-функционал для
ускорения работы, поскольку пишуться на серверном языке, а JavaScript используют лишь для
базовых вещей таких, как **ajax**, и это еще хорошо, если данные пересылаються в **json** а не 
кусками **html**-кода)
...
- Клиент-серверная архитектура. Это следует из первого пункта, делать совсем всё что хочеться средствами браузера
не получиться. Но отсюда вытекает огромный плюс: разделение ответственности (логикой и графикой занимаються
...
- Код написан полностью на JavaScript, это значит, что части кода полностьтю переиспользуються, и на клиенте, и на сервере,
что заставляет серьёзнее относиться к тому, что пишешь. Проектировать код так, что бы он максимально не зависел от
среды, в которой выполняеться, будь-то браузер или node.js.
...
и соответственно, увеличить скорость обмена данными (что являеться самым узким звеном в цепочке).

### Отличия

Главным отличием являеться двухпанельный интерфейс с привычными сочетаними клавишь. Но это не всё.
В противовес другим файловым менеджерам, работающим в браузере, Cloud Commander обладает консолью и редактором.

Редактор поддерживает подсветку синтаксиса больше 110 языков. Формат файла определяеться по его расширению.
...
Важным отличием так же являеться Node.js как платформа, на которой работает Файловый Менеджер.
...
- Используються стандартные диалоговые окна (`alert`, `confirm`, `prompt`).
...
После установки модулей прописанных в `package.json`, используються:
...
### Общие модули
Некоторые модули работают и на клиенте и на сервере, таким являеться [diff-match-patch](https://code.google.com/p/google-diff-match-patch/),
...
Что связываться с ним пока не очень хочеться, без особой нужды.
...
передача данных, запаковка (и распаковка на сервере) в свою очередь выполняеться чрезвычайно быстро.
...
И, если хеш изменился (без нашего ведома), файл загружаеться снова, в другом случае хеш обновляеться при каждомом сохранении файла.
...
Клиентские модули загружаються по мере необходимости. Если человеку нужно меню - оно загружаеться,
...
### Join
Потоки `node.js` - очень мощный инструмент, который кардинально отличаеться от того, что есть в других
...
в скорости, но нет. Всё работает как часы, и особых замедлений не ощущаеться, а в местое этого, появляеться
...
И, мне кажеться, это правильное направление.
...
и на быстродействие сервера это не должно особо влияеть, поскольку файлы читаються последовательно,
но сразу после чтения отдаються клиенту.

Если же прочитать файл не удаёться, вместо него отправляеться сообщение об ошибке, после чего поток закрываеться.
