...
Главным отличием является двухпанельный интерфейс с привычными сочетаниями клавишь. Но это не всё.
...
- При отсутствии браузера с сервером можно будет общаться только по средством HTTP-запросов.
...
- Отсутствует прогрес бар при копировании/перемещении/удалении файлов.
...
Поэтому, если, между правками, файл на сервере не менялся - отправляется патч и применяеться на сервере (при условии, что файл весит не очень много,
...
```
С помощью символа ":" имена файлов отделяються друг-от-друга, таким образом, объединять можно абсолютно что угодно,
