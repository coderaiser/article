...
Он состоит из нескольких маленьких модулей, каждый из которых упрощает и автоматизирует рутинную работу программиста,
такую как:

- генерация `ChengeLog`
- обновление версии в `package.json` и `bower.json`
- добавление тега
- публикация в npm
- создание релиза на GitHub

Все эти задачи легко можно было реализовать используя `Gulp`. Но копируя очередной раз `gulpfile.js` в новый проект,
...
Мне часто приходится обьяснять разработчикам преимущества модульной разработки и иногда это дается очень сложным путем.
